...
public class RequestWrapper implements StageInterface {
    ...
    public RequestWrapper(Request request,BaseInfo baseInfo) {
        super();
        this.request = request;
        this.baseInfo = baseInfo;
    }

    public static String getNextStage(BaseInfo baseInfo) throws IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        Object stage = null;
        int index = Arrays.asList(stages).indexOf(baseInfo.getStage());
        do {
            index ++;
            stage = PropertyUtils.getProperty(baseInfo,"stage" + stages[index]);
        } while (stage == null);

        return stages[index];
    }

    public static String getPreviousStage(BaseInfo baseInfo) throws IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        Object stage = null;
        int index = Arrays.asList(stages).indexOf(baseInfo.getStage());
        do {
            index --;
    ...
    public String getPreviousStage() {
        try {
            return getPreviousStage(baseInfo);
