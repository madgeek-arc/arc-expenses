...
function post_resourceType {
	data=`cat $1`
	response=$(curl -X POST --write-out %{http_code} --silent --output /dev/null --data "$data" --header "Content-Type:application/json"  http://$2:8010/arc-expenses-service/resourceType)
